...
region_coords = pd.read_csv("clustered_coordinates.csv")
...
def predict_and_visualize_on_single_map(model, device, random_images):
    """
    ...
    for img_num in random_images:
        ...
        predicted_region_index = predict_region(model, input_image, device)

        # Get the predicted coordinates by mapping the region index
        predicted_coords = get_coordinates_from_region(predicted_region_index)
        ...
        folium.Marker(
            location=predicted_coords,
            popup=f"Predicted Location (Image {img_num})",
            icon=folium.Icon(color="red", icon="info-sign"),
        ).add_to(m)
